"""Skip guard for O*NET tests that need a live Neo4j.

These tests reload the fixture with ``wipe=True``, so they delete this suite's
nodes in whatever database ``ONET_NEO4J_URI`` / ``NEO4J_URI`` points at. The
...
SKIP_REASON = "Neo4j not available (see src/ta_taxonomies/suites/onet/NOTES.md)"


def neo4j_available() -> bool:
    if not (os.getenv("ONET_NEO4J_PASSWORD") or os.getenv("NEO4J_PASSWORD")):
        return False
    try:
